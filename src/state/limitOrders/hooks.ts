...
import { LimitOrdersState } from './reducer'
import { useUserSlippageTolerance } from '../user/hooks'

export function useLimitOrdersState(): AppState['limitOrders'] {
  return useSelector<AppState, AppState['limitOrders']>(state => state.limitOrders)
}

export function useLimitOrdersActionHandlers(): {
...
export function tryParseAmount(value?: string, currency?: Currency): CurrencyAmount | undefined {
  ...
  try {
    ...
    if (currency instanceof Token) {
      const token = new TokenAmount(currency, JSBI.BigInt(typedValueParsed));
    }
...
export function useDerivedLimitOrdersInfo(): {
...
export function queryParametersToLimitOrdersState(parsedQs: ParsedQs): LimitOrdersState {
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    if (!chainId) return
    const parsed = queryParametersToLimitOrdersState(parsedQs)
